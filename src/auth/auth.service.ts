  ...
  async recoveryPassword(userAuth: UserRecoveryDTO): Promise<ResponseMinimalDTO> {
    ...
    try {
      ...
      await transporter.sendMail({
        from: '"OpenboxCloud" <no-reply@openbox.cloud>', // sender address
        to: `${user.email}`, // list of receivers
        subject: 'Recuperación de contraseña', // Subject line
        text: `recuperación de contraseña, ${url}`, // plain text body
...
`, // html body
      ...
      console.error(error);
      message = 'No se ha podido enviar el correo de verificación intenta nuevamente mas tarde.';
    }
