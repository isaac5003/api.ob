...
router.get('/general', async (req, res) => {
  ...
  try {
    const { startDate, endDate, documentType, customer, invoicesSeller, invoicesZone, status, service } = req.query;

    let documentTypes = req.conn.getRepository('InvoicesDocumentType').createQueryBuilder('idt');
    ...
    console.error(error);
    return res.status(500).json({ message: 'Error al obtener el reporte de ventas general.' });
  }
});
