...
router.put('/void/:id', async (req, res) => {
  ...
  if (!statuses.includes(invoice.status.id)) {
    return res.status(500).json({ message: 'El documento no puede ser anulado porque tiene un estado que no lo permite.' });
  }
  ...
  try {
    ...
    return res.status(500).json({
      message:
        "Error al anular el documento. Contacta con tu administrador.",
    });
  }
});
...
router.put('/reverse/:id', async (req, res) => {
  ...
  const statuses = [2, 3]
  if (!statuses.includes(invoice.status.id)) {
    return res.status(500).json({ message: 'El documento no puede revertirse porque tiene un estado que no lo permite.' });
  }
  ...
  try {
    ...
    return res.status(500).json({
      message:
        "Error al revertir el documento. Contacta con tu administrador.",
    });
  }
});
