...
router.put("/void/:id", async (req, res) => {
  ...
  if (!statuses.includes(invoice.status.id)) {
    return res.status(500).json({
      message:
        "El documento no puede ser anulado porque tiene un estado que no lo permite.",
    });
  }
...
router.put("/reverse/:id", async (req, res) => {
  ...
  if (!statuses.includes(invoice.status.id)) {
    return res.status(500).json({
      message:
        "El documento no puede revertirse porque tiene un estado que no lo permite.",
    });
  }
...
router.put("/printed/:id", async (req, res) => {
  ...
  if (!statuses.includes(invoice.status.id)) {
    return res.status(500).json({
      message:
        "El documento no puede marcarse como impreso porque tiene un estado que no lo permite.",
    });
  }
  ...
  try {
    ...
    return res.status(500).json({
      message:
        "Error al marcar como impreso el documento. Contacta con tu administrador.",
    });
  }
});
